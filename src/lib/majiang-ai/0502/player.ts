...
export default class Player extends Majiang.Player {
  qipai(qipai) {
    this._defen_cache = {};
    this._eval_cache = {};
  ...
  select_gang(info) {
    ...
    if (n_xiangting < 3) {
      ...
      for (let m of this.get_gang_mianzi(this.shoupai)) {
        ...
        if (info) {
          let p = m.match(/\d{4}$/) ? m.substr(0, 2) : m[0] + m.substr(-1);
  ...
  select_dapai(info) {
    ...
    for (let p of this.get_dapai(this.shoupai).reverse().sort(cmp)) {
      ...
      if (info) {
        info.map((i) => {
          if (i.p == p.substr(0, 2) && i.m) i.weixian = weixian && weixian(p);
        });
  ...
  select_daopai() {
    return (
      this._model.shan.paishu == 0 && Majiang.Util.xiangting(this.shoupai) == 0
    );
  }
  ...
  get_defen(shoupai, rongpai) {
    ...
    if (rongpai) paistr = paistr.replace(/^(.*?)(,.*|\*?)$/, `$1${rongpai}$2`);
  ...
  eval_shoupai(shoupai, paishu, back) {
    ...
    if (n_xiangting == -1) {
      ...
      for (let p of add_hongpai(Majiang.Util.tingpai(shoupai))) {
        ...
        let ev = this.eval_shoupai(new_shoupai, paishu, back);
        if (!back) {
          if (n_xiangting > 0) ev += this.eval_fulou(shoupai, p, paishu, back);
        }
      ...
      for (let p of add_hongpai(this.tingpai(shoupai))) {
        ...
        rv += paishu[p.substr(0, 2)] * (p[2] == "+" ? 4 : p[2] == "-" ? 2 : 1);
      }
    }
